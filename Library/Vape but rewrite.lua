...
local PresetColor = Color3.fromRGB(255, 117, 56)
local CloseBind = Enum.KeyCode.RightControl

if game:GetService("CoreGui"):FindFirstChild("Vaperewrite") then
   game:GetService("CoreGui"):FindFirstChild("Vaperewrite"):Destroy()
end
...
function lib:Window(text, preset, closebind)
    ...
    PresetColor = preset or Color3.fromRGB(255, 117, 56)
    fs = false
    ...
    BackgroundImage.Image = "rbxassetid://98921512564349" -- Replace with your image asset ID
    BackgroundImage.Size = UDim2.new(0, 285, 0, 285) -- Covers the entire screen
    BackgroundImage.Position = UDim2.new(0.5, -142.5, 0, 0)
    BackgroundImage.ZIndex = 0 -- Set ZIndex to be behind all other elements
    BackgroundImage.BackgroundTransparency = 1 -- Transparent background for the image
    BackgroundImage.ScaleType = Enum.ScaleType.Fit -- Ensures the image maintains its aspect ratio
...
local textWidth = game:GetService("TextService"):GetTextSize(Title.Text, Title.TextSize, Title.Font, Vector2.new(Title.Size.X.Offset, Title.Size.Y.Offset)).X
...
Hub.Position = UDim2.new(Title.Position.X.Scale, Title.Position.X.Offset + textWidth + 5, Title.Position.Y.Scale, Title.Position.Y.Offset)
Hub.Size = UDim2.new(0, 200, 0, 23)
Hub.Font = Enum.Font.GothamSemibold
Hub.Text = "HUB" -- Change this to the desired text
Hub.TextColor3 = Color3.fromRGB(255, 117, 56)
Hub.TextSize = 12.000
Hub.TextXAlignment = Enum.TextXAlignment.Left
    ...
    function tabhold:Tab(text)
        ...
        coroutine.wrap(
            function()
                while wait() do
                    TabBtnIndicator.BackgroundColor3 = Color3.fromRGB(255, 117, 56)
                end
            end
        )()
...
Tab.CanvasSize = UDim2.new(0, 0, 0, TabLayout.AbsoluteContentSize.Y)
        ...
        function tabcontent:Toggle(text,default, callback)
            ...
            FrameToggle3.BackgroundColor3 = Color3.fromRGB(255, 117, 56)
...
function tabcontent:Slider(text, min, max, start, callback)
    ...
    local SliderValue = Instance.new("TextBox") -- Changed to TextBox
    ...
    SliderValue.ClearTextOnFocus = false -- Keep the text when editing
    ...
    SlideCircle.ImageColor3 = Color3.fromRGB(255, 117, 56)

    coroutine.wrap(
        function()
            while wait() do
                CurrentValueFrame.BackgroundColor3 = Color3.fromRGB(255, 117, 56)
                SlideCircle.ImageColor3 = Color3.fromRGB(255, 117, 56)
            end
        end
    )()
    ...
    -- Allow user to type in value
    SliderValue.FocusLost:Connect(function(enterPressed)
        if enterPressed then
            local typedValue = tonumber(SliderValue.Text)
            if typedValue and typedValue >= min and typedValue <= max then
                local scale = (typedValue - min) / (max - min)
                CurrentValueFrame.Size = UDim2.new(scale, 0, 0, 3)
                SlideCircle.Position = UDim2.new(scale, -6, -1.30499995, 0)
                pcall(callback, typedValue)
            else
                SliderValue.Text = tostring(math.floor((start / max) * (max - min) + min)) -- Reset to last value
            end
        end
    end)

    -- Handling InputBegan for both mouse and touch
    ...
    -- Handling InputEnded for both mouse and touch
    ...
    -- Handle dragging for both mouse and touch input
    ...
    Tab.CanvasSize = UDim2.new(0, 0, 0, TabLayout.AbsoluteContentSize.Y)
        ...
        function tabcontent:Textbox(text, disapper, callback)
            ...
            TextboxFrame.BackgroundColor3 = Color3.fromRGB(255, 117, 56)
